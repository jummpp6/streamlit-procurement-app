...
def render_space_fourcolor_dialog(
    ...
    **kwargs,
    ...
    st.markdown("---")
    st.write(
        "ตรวจสอบและแก้ไขรายการสินค้า จำนวน (รองรับทศนิยม) และราคา เพื่อใช้สร้างเอกสารสี่สีและรายละเอียดคุณลักษณะ"
    )
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "quantity": st.column_config.NumberColumn(
                "จำนวน",
                min_value=0.0,
                step=0.01,  # 🟢 รองรับทศนิยม เช่น 1.5 หรือ 0.25
                format="%.2f",  # 🟢 แสดงทศนิยม 2 ตำแหน่ง
                default=1.0,
                required=True,
            ),
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "จำนวน": st.column_config.NumberColumn(
                format="%.2f"
            ),  # 🟢 พรีวิวตารางแสดงทศนิยม
...
# ฟังก์ชันสำหรับกรณีเข้าใช้งานแบบหน้าเว็บปกติ (เผื่อใช้หน้าหลักเดี่ยวๆ)
def render_space_4color_page():
    ...
    with col_rec1:
        receiver = st.selectbox(
            "ผู้รับพัสดุ (หลัก)", options=receiver_options, key="sc_receiver"
        )
    with col_rec2:
        receiver_sub = st.selectbox(
            "ผู้รับพัสดุแทน",
    ...
    st.write(
        "กรอกรายการสินค้า จำนวน (รองรับทศนิยม) และราคาต่อหน่วยตามต้องการ"
    )
    ...
    edited_df = st.data_editor(
        st.session_state.sc_items_df,
        num_rows="dynamic",
        use_container_width=True,
        hide_index=True,
        key="sc_data_editor_table",
        column_config={
            "name": st.column_config.Column("รายการ", width="large", required=True),
            "quantity": st.column_config.NumberColumn(
                "จำนวน",

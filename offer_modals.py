...
def edit_address_modal_content(shop_name, c_address, c_phone, c_tax_id):
  ...
  new_address = st.text_area(
      ...
      height=70,
      placeholder=(
          "ตัวอย่าง: 76,77,78-79 ซอยศรีสุริโยทัย 1 ต.ทะเลชุบศร อ.เมือง จ.ลพบุรี"
          " 15000"
      ),
  )
  ...
  with col1:
    if st.button("บันทึกการแก้ไข", type="primary", use_container_width=True):
      ...
      idx = df_shops[
          df_shops[name_col].astype(str).str.strip() == str(shop_name).strip()
      ].index

      if not idx.empty:
        df_shops.loc[
            idx[0],
            "address" if "address" in df_shops.columns else df_shops.columns[1],
        ] = new_address
        df_shops.loc[
            idx[0],
            "phone" if "phone" in df_shops.columns else df_shops.columns[2],
        ] = clean_phone_val
        df_shops.loc[
            idx[0],
            "tax_id" if "tax_id" in df_shops.columns else df_shops.columns[3],
        ] = clean_tax_val
      else:
      ...
      if save_shops_data(df_shops):
        ...
        st.error(
            "⚠️ ไม่สามารถบันทึกข้อมูลลง Google Sheets ได้ กรุณาตรวจสอบการเชื่อมต่อ"
        )
